...
def search():
    ...
    offset = int(request.args.get("offset", "0")) if request.args.get("offset", "").isdigit() else 0
    key = query.lower()
    
    if key in _search_cache:
        tracks = _search_cache[key]
        return jsonify({"tracks": tracks[offset:offset+20]})

    try:
        out, _ = ytdlp(
            f"ytsearch40:{query}", "--flat-playlist",
