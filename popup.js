...
async function playTrack(index) {
  ...
  // Mark as played immediately so it won't be suggested again
  playedIds.add(t.id);
  ...
  // Track current video for radio mix
  currentVideoId = t.id;
...
// ── Context tracking — remember current search/category ─────────────────────────
let currentSearchQuery = '';     // Track search query for continuous flow
let currentMode = 'browse';      // 'search' or 'radio' (video-based)
let currentVideoId = '';         // For radio mix mode

// ── Radio queue — pre-loaded related songs from yt-dlp (zero API quota) ──────
let radioQueue = [];
let searchQueueOffset = 0;       // For pagination of search results
...
async function loadMoreSearchResults() {
  ...
  try {
    const res = await fetch(
      `http://127.0.0.1:7842/search?q=${encodeURIComponent(currentSearchQuery)}&offset=${searchQueueOffset}`
    );
    if (!res.ok) return [];
    const data = await res.json();
    searchQueueOffset += (data.tracks?.length || 0);
    // Filter out anything already played
    return (data.tracks || []).filter(t => !playedIds.has(t.id));
  } catch {
    return [];
  }
}

// ── Next / Prev — smart context-aware selection ────────────────────────────────

// Extract the core song name (very strict - remove artist, all variations)
const baseTitle = t => {
  let title = t.title
    .replace(/\(.*?\)|\[.*?\]/gi, '')                    // Remove all brackets/parens
    .replace(/(-|–|—)\s*(lyrics|official|video|audio|slowed|reverb|hd|4k|full|session|acoustic|cover|remix|version|edition|remaster|extended|radio edit|mix).*$/gi, '')  // Remove all variation markers
    .replace(/\s+(by\s+)?[\w\s&]+(?:remix|version|edit)$/gi, '')  // Remove remix/version at end
    .split(/\s*(?:featuring|ft\.|feat\.|\|)/gi)[0]     // Take first part before featuring
    .trim()
    .toLowerCase();
  
  // Further normalize - remove common words that might differ
  title = title.replace(/\s+(song|track|tune|number)\s*$/gi, '');
  return title;
};

// Check if two tracks are essentially the same song
const isSameSong = (t1, t2) => {
  const base1 = baseTitle(t1);
  const base2 = baseTitle(t2);
  
  // Exact match
  if (base1 === base2) return true;
  
  // If either is very short, check for substring (avoid false positives)
  if (base1.length < 5 || base2.length < 5) return base1 === base2;
  
  // Check for 80%+ similarity (handles typos)
...
async function playNext() {
  ...
  // First: Find next genuinely DIFFERENT song in current queue
  for (let i = currentIndex + 1; i < queue.length; i++) {
    if (!isSameSong(currentTrack, queue[i]) && !playedIds.has(queue[i].id)) { 
      playedIds.add(queue[i].id);
      playTrack(i); 
      return; 
    }
  }

  // Second: Pull from pre-loaded radioQueue (already different songs from YouTube mix)
  if (radioQueue.length) {
    let next = radioQueue.shift();
    // Make sure it's not the same song
    while (radioQueue.length && isSameSong(currentTrack, next)) {
      next = radioQueue.shift();
    }
    if (!isSameSong(currentTrack, next) && !playedIds.has(next.id)) {
      queue.push(next);
  ...
  // Third: In search mode, fetch new results and use radio mix for better variety
  if (currentMode === 'search' && currentVideoId) {
    // Load radio from current song - better variety
    await loadRadioQueue(currentVideoId);
    if (radioQueue.length) {
      let next = radioQueue.shift();
      while (radioQueue.length && isSameSong(currentTrack, next)) {
        next = radioQueue.shift();
  ...
  // Fallback: just go to next song if available
...
async function search(query) {
  ...
  // Save search context for smart next
